...
import type { Project, Trade, Task, AppState, AITaskUpdate, ChangeRequest, AppUser } from './types';
import { TaskStatus } from './types';
import { MOCK_PROJECTS, MOCK_TRADES, MOCK_TASKS, MOCK_USERS } from './constants';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const initializeAuth = async () => {
            try {
                ...
                setAppState(prev => ({ ...prev, isLoading: false, error: `Authentication setup failed: ${error.message}`}));
    ...
    const setupDemoOrOfflineMode = () => {
        ...
        const initialTasks = MOCK_TASKS as { [projectId: string]: Omit<Task, 'id' | 'projectId'>[] };
    ...
    useEffect(() => {
        ...
        if (process.env.API_KEY) {
            try {
             ...
             console.warn("Gemini API_KEY not found. Chat and AI features will be disabled.");
        }
    ...
    const seedData = async (dbInstance: firebase.firestore.Firestore, currentUserId: string) => {
        ...
        if (projectsSnap.empty || usersSnap.empty) {
            ...
            if (projectsSnap.empty) {
                ...
                MOCK_PROJECTS.forEach(project => {
                    ...
                    const tasksForProject = MOCK_TASKS[project.id as keyof typeof MOCK_TASKS] || [];
                    tasksForProject.forEach(task => {
                        const taskColRef = dbInstance.collection(`projects/${project.id}/tasks`);
    ...
    useEffect(() => {
        if (appState.isOffline || !isAuthenticated || !appState.db || appState.error) {
        ...
        const unsubscribeProjects = projectsQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
            const projectsData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Project));
            setAppState(prev => ({ ...prev, projects: projectsData, isLoading: snapshot.metadata.hasPendingWrites ? prev.isLoading : false }));
        }, (error) => {
            console.error("Error fetching projects:", error);
            setAppState(prev => ({ ...prev, isLoading: false, error: "Failed to fetch project data." }));
        });

        const tradesQuery = appState.db.collection('trades');
        const unsubscribeTrades = tradesQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
        ...
        const unsubscribeUsers = usersQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
    ...
    useEffect(() => {
        ...
        const tasksUnsubscribers = appState.projects.map(project => {
            const tasksQuery = appState.db!.collection(`projects/${project.id}/tasks`);
            return tasksQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
                const tasksData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Task));
                setAppState(prev => ({
                    ...prev,
                    tasks: { ...prev.tasks, [project.id]: tasksData },
                }));
            }, (error) => console.error(`Error fetching tasks for project ${project.id}:`, error));
        });
    ...
    const handleSelectUserView = (user: AppUser | null) => {
        setAppState(prev => {
            ...
            switch (role) {
                case 'Customer':
                    projectsForView = prev.projects.filter(p => p.client === user!.name);
                ...
                case 'Subcontractor':
                    ...
                    break;
                default: // PM or All
                    projectsForView = user ? prev.projects.filter(p => p.pmId === user.id) : prev.projects;
                    const isSelectedProjectVisible = prev.selectedProject && projectsForView.some(p => p.id === prev.selectedProject!.id);
                    newSelectedProject = isSelectedProjectVisible ? prev.selectedProject : (projectsForView[0] || null);
                    break;
            }
    ...
    const handleTaskUpdate = async (projectId: string, taskId: string, update: Partial<Task> & Partial<AITaskUpdate>) => {
        const { delayDurationInDays, delayReason, ...taskUpdate } = update;

        if(taskUpdate.status === TaskStatus.Completed && taskUpdate.progress !== 100) taskUpdate.progress = 100;
        if(taskUpdate.status === TaskStatus.NotStarted && taskUpdate.progress !== 0) taskUpdate.progress = 0;

        if (delayDurationInDays && delayDurationInDays > 0 && taskUpdate.status === 'Delayed') {
            const trade = appState.trades.find(t => t.id === (appState.tasks[projectId].find(tsk => tsk.id === taskId) as Task).tradeId);
            ...
            if (appState.isOffline || !appState.db) {
                ...
                const taskRef = appState.db.collection('projects').doc(projectId).collection('tasks').doc(taskId);
                await taskRef.update(taskUpdate as { [x: string]: any });
            }
        }
    };
    
    const handleCreateProject = async (projectData: Omit<Project, 'id' | 'pmId'>, selectedSubcontractorIds: string[]) => {
        const today = new Date().toISOString().split('T')[0];

        const pmId = (appState.viewingAsUser?.role === 'Project Manager'
            ? appState.viewingAsUser.id
            : appState.users.find(u => u.role === 'Project Manager')?.id) || 'unassigned';
        ...
        if (appState.isOffline || !appState.db) {
            ...
            tradeIdsForNewTasks.forEach(tradeId => {
                const taskColRef = appState.db!.collection(`projects/${newProjectRef.id}/tasks`);
                const newTaskRef = taskColRef.doc();
    ...
    const handleAddSubcontractor = async (data: { name: string; tradeType: string; phone: string; email: string; }) => {
        if (appState.isOffline || !appState.db) {
    ...
    const handleApproveChangeRequest = async (request: ChangeRequest) => {
        ...
        await handleCascadingUpdate(request.projectId, request.taskId, request.proposedUpdate.delayDurationInDays, {
    ...
    const handleDenyChangeRequest = (requestId: string, projectId: string) => {
        setAppState(prev => ({
            ...
            changeRequests: {
                ...prev.changeRequests,
                [projectId]: (prev.changeRequests[projectId] || []).filter(r => r.id !== requestId),
            },
        }));
    };

    const handleCascadingUpdate = async (projectId: string, rootTaskId: string, delayDays: number, rootUpdate: Partial<Task>) => {
        ...
        projectTasks.forEach(t => {
            if (t.dependency) {
                if (!dependencyMap.has(t.dependency)) dependencyMap.set(t.dependency, []);
        ...
        const newStartDate = new Date(originalStartDate.setDate(originalStartDate.getDate() + delayDays));
        const newEndDate = new Date(originalEndDate.setDate(originalEndDate.getDate() + delayDays));
        ...
        while (queue.length > 0) {
            ...
            const depNewEndDateStr = updatesToApply[dependencyTask.id]?.endDate || dependencyTask.endDate;
            ...
            const originalDuration = (new Date(currentTask.endDate).getTime() - new Date(currentTask.startDate).getTime()) / (1000 * 3600 * 24);
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-100 p-4">
                <div className="text-center p-8 bg-white shadow-xl rounded-lg max-w-lg border border-red-200">
                    ...
                    <h2 className="mt-4 text-2xl font-bold text-slate-800">Application Error</h2>
                    <p className="mt-2 text-base text-slate-600">{appState.error}</p>
                    <p className="mt-6 text-sm text-slate-500">
                        This is often a configuration issue. Please check the error message for instructions.
                    </p>
                </div>
            </div>
        );
    ...
    if (role === 'Subcontractor') {
        const subProjects = appState.projects.filter(p =>
            (appState.tasks[p.id] || []).some(t => t.tradeId === viewingAsUser!.tradeId)
        );
        return (
            <SubcontractorView
                user={viewingAsUser!}
    ...
    const subcontractors = appState.users.filter(u => u.role === 'Subcontractor');
    const handleLogout = () => {
        if (isDemo) {
    ...
    return (
        <div className="bg-slate-100 min-h-screen font-sans text-slate-800 flex flex-col lg:flex-row">
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
                {appState.isOffline && (
                    <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded-r-lg mb-6 shadow-md no-print" role="alert">
                        <div className="flex items-center gap-3">
                            <AlertTriangle className="h-6 w-6 text-yellow-600 flex-shrink-0" />
                            <div>
                                <h3 className="font-bold">{isDemo ? 'Demo Mode Active' : 'Offline Mode Active'}</h3>
                                <p className="text-sm">
                                    {isDemo 
                    ...
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            ...
                            <h2 className="mt-4 text-xl font-medium text-slate-600">
                                { viewingAsUser ? `No Projects for ${viewingAsUser.name}` : 'No Projects Found' }
                            </h2>
                            <p className="mt-1 text-sm text-slate-500">{appState.isOffline ? 'Running in offline mode.' : 'Select a different user view or create a new project.'}</p>
                        </div>
                    </div>
                )}
            </main>
