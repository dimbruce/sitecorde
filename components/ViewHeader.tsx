...
const ViewHeader: React.FC<ViewHeaderProps> = ({ user, allUsers, onSelectUserView }) => {
  ...
  return (
    <header className="bg-white shadow-sm p-4 border-b border-slate-200">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div>
          <label className="text-xs text-slate-500 font-semibold tracking-wide block text-right mb-1">VIEWING AS</label>
                  ...
                  groupedUsers[role] && (
                    <div key={role}>
                      ...
                      {groupedUsers[role].map(u => (
                         <button
                           key={u.id}
                           onClick={() => { onSelectUserView(u); setIsUserMenuOpen(false); }}
                           className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 flex items-center justify-between"
                         >
                          {u.name}
                          {user.id === u.id && <Check size={16} className="text-blue-600" />}
                         </button>
                      ))}
                    </div>
                  )
