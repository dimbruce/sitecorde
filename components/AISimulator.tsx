...
const AISimulator: React.FC<AISimulatorProps> = ({ project, trades, tasks, onTaskUpdate, isOffline }) => {
  ...
  const handleParseMessage = async () => {
    ...
    if (isOffline) {
        setFeedback({ type: 'error', text: 'AI Simulator is disabled in offline mode.' });
    ...
    try {
      ...
      if (taskToUpdate) {
        ...
        const tradeName = trades.find(t => t.id === result.tradeId)?.name || 'Unknown Trade';
        let feedbackText;
        if (result.delayDurationInDays && result.status === 'Delayed') {
            feedbackText = `Delay of ${result.delayDurationInDays} days for ${tradeName} detected. A change request has been created for your approval.`
             ...
             if(result.progress !== undefined) feedbackText += ` Progress set to ${result.progress}%.`
        }
        setFeedback({ type: 'success', text: feedbackText });
        setMessage('');
      } else {
        throw new Error(`Could not find a task associated with trade ID ${result.tradeId} in this project.`);
      }
    } catch (error: any) {
      setFeedback({ type: 'error', text: error.message || 'An unknown error occurred.' });
    } finally {
  ...
  return (
    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
      <h3 className="text-lg font-semibold mb-2 text-slate-800">AI Subcontractor Update (Simulation)</h3>
      <p className="text-sm text-slate-500 mb-4">
        Simulate an SMS update. The AI parses it, updates status, progress, and creates change requests for delays.
        <br />
        Examples: <span className="font-mono bg-slate-100 p-1 rounded-md text-xs">Framing is 50% complete</span> or <span className="font-mono bg-slate-100 p-1 rounded-md text-xs">Plumbing is delayed by a week due to parts.</span>
      </p>
      ...
      {feedback && (
        <div className={`mt-3 text-sm p-3 rounded-lg ${feedback.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          {feedback.text}
        </div>
      )}
    </div>
