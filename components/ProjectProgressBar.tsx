...
import { Layers, Home, Wrench, ShieldCheck, Wind, Paintbrush, Sparkles, Check } from 'lucide-react';
...
const MILESTONE_DEFINITIONS = [
    ...
    { name: 'Rough-ins', icon: Wrench, tradeNames: ['Plumbing (Rough-in)', 'Electrical (Rough-in)', 'HVAC (Rough-in)'] },
    { name: 'Inspections', icon: ShieldCheck, tradeNames: ['Municipal Inspector'] },
...
const getMilestones = (tasks: Task[], trades: Trade[]): ProjectMilestone[] => {
    ...
    const milestones: ProjectMilestone[] = MILESTONE_DEFINITIONS.map(def => {
        ...
        if (milestoneTasks.length > 0) {
            const areAllComplete = milestoneTasks.every(t => t.status === TaskStatus.Completed);
            const isAnyInProgress = milestoneTasks.some(t => t.status === TaskStatus.InProgress || t.status === TaskStatus.Delayed);
    ...
    // Assign "Next" status
    if (!hasInProgress) {
        const firstIncompleteIndex = milestones.findIndex(m => m.status === MilestoneStatus.Incomplete);
        if (firstIncompleteIndex !== -1) {
            milestones[firstIncompleteIndex].status = MilestoneStatus.Next;
             ...
             const allBeforeFinalAreComplete = milestones.slice(0, -1).every(m => m.status === MilestoneStatus.Completed);
    ...
    if (finalMilestone && finalMilestone.status === MilestoneStatus.Incomplete) {
        const allOthersComplete = milestones.slice(0, -1).every(m => m.status === MilestoneStatus.Completed);
...
const MilestoneStep: React.FC<{ milestone: ProjectMilestone; isFirst: boolean; isLast: boolean }> = ({ milestone, isFirst, isLast }) => {
    const isCompleted = milestone.status === MilestoneStatus.Completed;
    const isInProgress = milestone.status === MilestoneStatus.InProgress || milestone.status === MilestoneStatus.Next;

    const circleBaseClass = "w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 z-10";
    const circleColorClass = isCompleted ? 'bg-blue-600 text-white' : isInProgress ? 'bg-blue-100 text-blue-600 ring-4 ring-blue-200' : 'bg-slate-200 text-slate-500';
    const pulseClass = milestone.status === MilestoneStatus.InProgress ? 'animate-pulse' : '';
    ...
    return (
        <div className="flex flex-col items-center flex-1">
            ...
            <p className={`mt-2 text-center text-xs sm:text-sm font-semibold ${isCompleted || isInProgress ? 'text-slate-800' : 'text-slate-500'}`}>{milestone.name}</p>
        </div>
    );
};


const ProjectProgressBar: React.FC<ProjectProgressBarProps> = ({ tasks, trades }) => {
    const milestones = useMemo(() => getMilestones(tasks, trades), [tasks, trades]);

    return (
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="text-base font-semibold text-slate-800 mb-6 text-center">Project Milestones</h3>
            <div className="flex items-start">
                {milestones.map((milestone, index) => {
                    const isLast = index === milestones.length - 1;
                    const nextMilestone = isLast ? null : milestones[index + 1];
                    const isConnectorActive = milestone.status === MilestoneStatus.Completed;
                    
                    return (
                        <div key={milestone.name} className="flex-1 flex items-center">
                            <MilestoneStep milestone={milestone} isFirst={index === 0} isLast={isLast} />
                            {!isLast && (
