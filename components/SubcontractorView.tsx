...
interface SubcontractorViewProps {
    ...
    trades: Trade[];
    onTaskUpdate: (projectId: string, taskId: string, update: Partial<Task>) => void;
    allUsers: AppUser[];
...
const SubcontractorView: React.FC<SubcontractorViewProps> = (props) => {
  ...
  const visibleTasks = useMemo(() => {
    ...
    return projectTasks
      .filter(task => 
          task.tradeId === subcontractorTradeId || task.status === TaskStatus.Completed
      )
  ...
  return (
    <div className="bg-slate-100 min-h-screen w-full flex flex-col">
      <ViewHeader user={user} allUsers={allUsers} onSelectUserView={onSelectUserView} />
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
            {selectedProject ? (
                <div>
                    ...
                    <p className="text-slate-500 mt-1 mb-6">{selectedProject.address}</p>
                                ...
                                {visibleTasks.length > 0 ? visibleTasks.map(task => {
                                    ...
                                    return (
                                        <TaskCard 
                                            ...
                                            task={task} 
                                            trade={trade} 
                                            onUpdateTask={(update) => onTaskUpdate(selectedProject.id, task.id, update)}
                                            onEditTask={isOwner ? () => alert("Editing not implemented in this view.") : undefined}
                                            hideContactInfo={!isOwner}
                                        />
                                    );
                                }) : <p className="text-slate-500 text-center py-4">No relevant tasks to display for this project.</p>}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                        ...
                        <h2 className="mt-4 text-xl font-medium text-slate-600">
                           {projects.length > 0 ? "Select a project to view tasks" : "No Projects Found"}
                        </h2>
                        <p className="mt-1 text-sm text-slate-500">Please select a project from the list on the left.</p>
                    </div>
                </div>
            )}
        </main>
