...
import type { Project, Task, Trade, AITaskUpdate, ChangeRequest } from '../types';
...
interface ProjectDashboardProps {
  ...
  onTaskUpdate: (projectId: string, taskId: string, update: Partial<Task> & Partial<AITaskUpdate>) => void;
...
const ProjectDashboard: React.FC<ProjectDashboardProps> = ({ project, tasks, trades, changeRequests, onTaskUpdate, onApproveRequest, onDenyRequest, isOffline }) => {
  ...
  useEffect(() => {
    if (prevTasksRef.current && prevTasksRef.current.length > 0 && tasks.length > 0) {
      ...
      const changedIds = tasks
        .filter(currentTask => {
          ...
          return prevTask && (currentTask.startDate !== prevTask.startDate || currentTask.endDate !== prevTask.endDate);
        })
        .map(t => t.id);
    ...
    <div className="space-y-6">
      ...
      <div className="no-print">
        <AISimulator 
          ...
          trades={trades} 
          tasks={tasks} 
          onTaskUpdate={(taskId, update) => onTaskUpdate(project.id, taskId, update)}
          isOffline={isOffline} 
        />
      </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-lg no-print">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1 rounded-md text-sm font-semibold transition-colors ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
              >
                <List size={16} /> List
              </button>
              <button
                onClick={() => setViewMode('timeline')}
                className={`flex items-center gap-2 px-3 py-1 rounded-md text-sm font-semibold transition-colors ${viewMode === 'timeline' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
              >
           ...
           <div className="p-4 bg-slate-50/50 task-list-container">
             <div className="space-y-3">
               {sortedTasks.length > 0 ? sortedTasks.map(task => {
                 ...
                 return <TaskCard key={task.id} task={task} trade={trade} onEditTask={() => handleEditTask(task)} onUpdateTask={(update) => onTaskUpdate(project.id, task.id, update)} />;
               }) : <p className="text-slate-500 text-center py-4">No tasks found for this project.</p>}
             </div>
           </div>
        ) : (
          <div className="timeline-view-container">
            <TimelineView tasks={tasks} trades={trades} onSelectTask={handleEditTask} />
          </div>
        )}
      </div>
