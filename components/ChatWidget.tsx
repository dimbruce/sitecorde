...
const ChatWidget: React.FC<ChatWidgetProps> = ({ chatService, isOffline }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage = "Sorry, I encountered an error. Please check the console for details or try again.";
      setMessages(prev => {
        const newMessages = [...prev];
        // If the last message is an empty AI placeholder, update it. Otherwise, add a new one.
        if (newMessages.length > 0 && newMessages[newMessages.length - 1].sender === 'ai' && newMessages[newMessages.length - 1].text === '') {
  ...
  return (
    <div className="no-print fixed bottom-6 right-6 h-[75vh] w-[90vw] max-w-md max-h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col border border-slate-200 z-50 transition-all duration-300 ease-out" role="dialog" aria-modal="true">
      {/* Header */}
      <header className="flex items-center justify-between p-4 border-b border-slate-200 bg-slate-50 rounded-t-2xl flex-shrink-0">
        <h3 className="font-bold text-lg text-slate-800">SiteCoord AI Assistant</h3>
        <div className="flex items-center gap-2">
             <button onClick={handleResetChat} className="p-2 rounded-full text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors" aria-label="Reset Chat">
                <RotateCcw size={18} />
             </button>
            <button onClick={() => setIsOpen(false)} className="p-2 rounded-full text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors" aria-label="Close chat">
                <X size={20} />
            </button>
        </div>
      </header>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50/50">
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.sender === 'ai' && <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">AI</div>}
            <div className={`max-w-xs md:max-w-sm px-4 py-2 rounded-2xl shadow-sm ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-slate-800 rounded-bl-none border border-slate-200'}`}>
              <p className="text-sm whitespace-pre-wrap">{msg.text}{isStreaming && index === messages.length - 1 && <span className="inline-block w-2 h-4 bg-slate-600 animate-pulse ml-1" />}</p>
            </div>
          </div>
        ))}
