...
const ChangeRequestPanel: React.FC<ChangeRequestPanelProps> = ({ requests, onApprove, onDeny }) => {
  ...
  return (
    <div className="bg-yellow-50 border-2 border-yellow-300 border-dashed p-5 rounded-xl space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h3 className="text-lg font-semibold text-slate-800">Pending Change Requests</h3>
      </div>
      <p className="text-sm text-slate-600">A subcontractor has reported a delay. Approve to automatically update the schedule for all dependent tasks.</p>
      
      <div className="space-y-3">
        {requests.map(request => (
          <div key={request.id} className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex-1">
                <p className="font-bold text-slate-800">{request.tradeName}</p>
                <p className="text-sm text-yellow-700 font-semibold mt-1">
                    Requests a {request.proposedUpdate.delayDurationInDays}-day delay.
                </p>
                <p className="text-sm text-slate-600 mt-2 pt-2 border-t border-slate-100">
                    <span className="font-semibold">Reason:</span> {request.proposedUpdate.delayReason || request.proposedUpdate.notes}
                </p>
            </div>
